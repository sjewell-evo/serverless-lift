...
const _ServerSideWebsite = class extends import_abstracts.AwsConstruct {
  ...
  createCacheBehaviors(bucket) {
    ...
    for (const [pattern, target] of Object.entries(this.getAssetPatterns())) {
      ...
      const filePath = typeof target === "string" ? target : target.target;
      let originBucket = new import_aws_cloudfront_origins.S3Origin(bucket);
      if (filePath.startsWith("s3://")) {
        ...
        if (existingBucketName in customOrigins) {
          ...
          existingBucket = import_aws_s3.Bucket.fromBucketName(this, existingBucketName, existingBucketName);
          customOrigins[existingBucketName] = existingBucket;
        }
        originBucket = new import_aws_cloudfront_origins.S3Origin(existingBucket, originProperties);
      }
      const cachePolicy = typeof target === "string" ? import_aws_cloudfront.CachePolicy.CACHING_OPTIMIZED : {
        cachePolicyId: import_aws_cdk_lib.Fn.ref(
